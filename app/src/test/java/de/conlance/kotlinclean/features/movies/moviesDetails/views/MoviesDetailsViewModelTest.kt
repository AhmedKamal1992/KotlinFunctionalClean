...
class MoviesDetailsViewModelTest: AndroidTest() {
    ...
    @Mock private lateinit var movieDetailsUseCaseImpl: MovieDetailsUseCaseImpl

    @Before fun setup() {
        movieDetailsUseCaseImpl = mock { onBlocking {run(any())}.thenReturn(Either.Right(movieDetails))}
        viewModel = MoviesDetailsViewModel(movieDetailsUseCaseImpl)
    }
